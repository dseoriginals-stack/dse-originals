...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* DRAWER */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-[420px] bg-white z-[101] shadow-2xl transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${isCartOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        <div className="flex flex-col h-full bg-[var(--bg-main)]">
          ...
          {cart.length > 0 && (
            <div className="bg-[var(--bg-card)] px-6 py-3 border-b flex items-center justify-between shrink-0">
              ...
              >
                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${isAllSelected ? "bg-[var(--brand-primary)] border-[var(--brand-primary)]" : "bg-white border-gray-200 group-hover:border-[var(--brand-soft)]"
                  }`}>
            ...
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  ...
                  className={`bg-[var(--bg-card)] cursor-pointer rounded-2xl p-4 flex gap-4 shadow-sm border border-[var(--border-light)] relative group transition-all ${selectedItems.includes(item.variantId) ? 'border-[var(--brand-primary)]' : 'hover:border-[var(--brand-soft)]/30'
                    }`}
                >
                  {/* CHECKBOX */}
                  <div className="flex items-center self-center">
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${selectedItems.includes(item.variantId) ? "bg-[var(--brand-primary)] border-[var(--brand-primary)]" : "bg-white border-gray-200"
                      }`}>
          ...
          {cart.length > 0 && (
            <div className="bg-[var(--bg-card)] px-6 pt-4 pb-8 border-t border-[var(--border-light)] shadow-[0_-10px_40px_rgba(0,0,0,0.03)] flex flex-col gap-4">
              ...
              <Link
                ...
                className={`group flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all duration-300 ${hasSelection
